...
const docs = eventBatch.map(
  (event: any) => new Document({pageContent: JSON.stringify(event), metadata: {id: event.game.videoTimeMs}})
);
...
What is the closes player to the ball in the frame with video time 7126784?
`;

const res = await chain.call({question: prompt2, chat_history: []});
console.log(res);
